...
const CompanyInfo = (props) => {
  ...
  useEffect(() => {
    if (intl.locale === "zh-CN") {
      if (keyType && keyType == 101) {
        setOneInfoTitle('公司介绍');
      } else if (keyType && keyType == 103) {
        setOneInfoTitle('股票详情');
      }
    } else {
      if (keyType && keyType == 101) {
        setOneInfoTitle('Company introduction');
      } else if (keyType && keyType == 103) {
        setOneInfoTitle('Stock details');
      }
    }
